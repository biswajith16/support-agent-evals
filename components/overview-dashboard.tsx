...
import { Activity, AlertTriangle, ArrowRight, ArrowUpRight, Bot, CheckCircle2, Clock3, Gauge, SearchCheck, ShieldAlert, Timer, UserRoundCheck } from "lucide-react";
...
const attentionRecords = evaluations.filter((record) => record.result !== "PASS");
const failedCount = attentionRecords.filter((record) => record.result === "FAIL").length;
const reviewCount = attentionRecords.filter((record) => record.result === "REVIEW").length;
...
export function OverviewDashboard() {
  ...
  const attentionQueueRef = useRef<HTMLDivElement>(null);
  function selectFeed(mode: "recent" | "attention") {
    setFeedMode(mode);
    window.setTimeout(() => attentionQueueRef.current?.scrollIntoView({ behavior: "smooth", block: "center" }), 0);
  }
  const feedRecords = useMemo(() => {
    const source = feedMode === "recent" ? [...evaluations].slice(-6).reverse() : attentionRecords.slice(-6).reverse();
    return source;
  }, [feedMode]);
  return (
    <div className="mx-auto max-w-[1600px] px-6 py-8 lg:px-10">
      <header className="mb-7 flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-sky-300">AI evaluation dashboard</p><h1 className="mt-2 text-3xl font-semibold tracking-tight text-slate-100">Support agent performance</h1><p className="mt-2 text-sm text-slate-400">Monitor the quality, reliability, and performance of AI support agents.</p></div><div className="flex flex-wrap items-center gap-2"><button onClick={() => selectFeed("recent")} className={`rounded-lg border px-3 py-2 text-xs transition ${feedMode === "recent" ? "border-sky-300/25 bg-sky-300/15 text-sky-100" : "border-transparent text-slate-400 hover:bg-slate-800"}`}>Latest activity</button><button onClick={() => selectFeed("attention")} className={`rounded-lg border px-3 py-2 text-xs transition ${feedMode === "attention" ? "border-rose-300/25 bg-rose-400/10 text-rose-100" : "border-transparent text-slate-400 hover:bg-slate-800"}`}>Needs attention <span className="ml-1 text-rose-300">{attentionRecords.length}</span></button><div className="flex items-center gap-2 rounded-lg border border-sky-300/15 bg-sky-300/5 px-3 py-2 text-xs text-sky-100"><span className="size-2 rounded-full bg-sky-300" /> {evaluations.length} simulated evaluations</div></div></header>

      <button onClick={() => selectFeed("attention")} className="group mb-5 flex w-full flex-col gap-3 rounded-xl border border-rose-400/20 bg-[linear-gradient(100deg,rgba(251,113,133,.1),rgba(16,27,46,.5)_48%,rgba(167,139,250,.08))] p-4 text-left transition hover:border-rose-300/40 hover:bg-rose-400/[.12] sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className="grid size-10 place-items-center rounded-lg bg-rose-400/15 text-rose-300"><AlertTriangle size={19} /></div><div><p className="text-sm font-semibold text-slate-100">Risk briefing ready</p><p className="mt-0.5 text-xs text-slate-400"><span className="text-rose-200">{failedCount} failures</span><span className="mx-1.5 text-slate-600">·</span><span className="text-amber-200">{reviewCount} records for review</span><span className="mx-1.5 text-slate-600">·</span>Open the priority queue to investigate.</p></div></div><span className="inline-flex items-center gap-1 text-xs font-medium text-rose-100">Review priority queue <ArrowRight size={15} className="transition group-hover:translate-x-1" /></span></button>
      ...
      <div ref={attentionQueueRef} className="scroll-mt-8 xl:col-span-3"><Panel title={feedMode === "recent" ? "Recent evaluations" : "Priority attention queue"} subtitle={feedMode === "recent" ? "Click any answer to investigate it" : `${attentionRecords.length} evaluations require a closer look · click a record to open the evidence`} className={`overflow-hidden transition-colors ${feedMode === "attention" ? "border-rose-400/40 bg-[#151a2b]" : ""}`}><div className="mb-4 flex items-center justify-between border-b border-[#25354e] pb-3"><span className={`text-xs font-medium ${feedMode === "attention" ? "text-rose-200" : "text-slate-500"}`}>{feedMode === "attention" ? "Showing highest-priority failures and reviews" : "Showing the most recently evaluated conversations"}</span>{feedMode === "attention" && <button onClick={() => selectFeed("recent")} className="text-xs text-sky-200 transition hover:text-sky-100">Back to latest</button>}</div><div className="overflow-x-auto"><table className="w-full min-w-[650px] text-left text-xs"><thead className="border-b border-[#25354e] text-[10px] uppercase tracking-wider text-slate-500"><tr><th className="pb-3 font-medium">ID</th><th className="pb-3 font-medium">Question</th><th className="pb-3 font-medium">Category</th><th className="pb-3 font-medium">Correctness</th><th className="pb-3 font-medium">Latency</th><th className="pb-3 font-medium">Result</th></tr></thead><tbody>{feedRecords.map((record) => <tr key={record.id} onClick={() => setSelectedEvaluation(record)} className={`cursor-pointer border-b border-[#1d2b42] transition last:border-0 ${feedMode === "attention" ? "hover:bg-rose-400/[.07]" : "hover:bg-sky-300/[0.05]"}`}><td className="py-3 font-mono text-slate-400">{record.id}</td><td className="max-w-48 truncate py-3 pr-4 text-slate-200">{record.question}</td><td className="py-3 pr-4 text-slate-400">{record.category}</td><td className="py-3 pr-4 text-slate-200">{record.correctness_score}%</td><td className="py-3 pr-4 text-slate-400">{formatLatency(record.latency_ms)}</td><td className="py-3"><ResultBadge result={record.result} /></td></tr>)}</tbody></table></div></Panel></div></div>
